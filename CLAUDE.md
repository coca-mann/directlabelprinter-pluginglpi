...
### Plugin bootstrap
- `setup.php` — declares `PLUGIN_DIRECTLABELPRINTER_VERSION` / min-max GLPI version. `plugin_init_directlabelprinter()` registers `$PLUGIN_HOOKS`: `csrf_compliant`, `Hooks::USE_MASSIVE_ACTION` unconditionally, and — only when the plugin is installed+active — `menu_toadd['directlabelprinter'] = ['config' => Menu::class]` (see "Menu and navigation" below; the key must be `'config'`, the real key GLPI 11 uses internally for the "Configurar" sector — `'setup'` is silently discarded), `secured_fields` (masks `printservers.api_key` in the UI), `add_javascript` (loads `js/components.js` on every page — see "Static assets" below for why the hook value has no `public/` prefix even though the file physically lives under `public/js/`), and `config_page` (`'front/printserver.php'` — the gear-icon shortcut next to the plugin's row in Configuração > Plug-ins; still points at the print-server list rather than the newer `front/config.php` hub).
- `hook.php` — `plugin_directlabelprinter_install()` creates 4 tables (`printservers`, `layouts`, `layout_itemtype`, `userprefs`) via raw SQL, contains an upgrade path (`elseif (!$DB->fieldExists($layouts_table, 'elements'))`) that drops and recreates the pre-redesign `layouts` table shape, defensively drops the legacy `glpi_plugin_directlabelprinter_auth` table, and (since 0.2.2) also inserts a `ttf` row into GLPI's own `glpi_documenttypes` table if missing — a clean GLPI install doesn't ship one, which silently blocks custom-font uploads in `Layout::attachCustomFontIfUploaded()`. `plugin_directlabelprinter_uninstall()` drops the 4 plugin tables (it does **not** remove the `ttf` document type — that's shared GLPI state, not plugin-owned). **`PLUGIN_DIRECTLABELPRINTER_VERSION` must match between `setup.php` and `hook.php`, and must be bumped whenever `install()`'s logic changes** — GLPI skips re-running `install()` on an existing install if the DB-recorded version already equals the constant.
