...
class ConfigController extends AbstractController {
    ...
    public function displayOrProcessConfig(Request $request): Response {
        ...
        if ($request->isMethod('POST')) {
            ...
            try {
                ...
                if ($request->request->has('test_connection')) {
                    ...
                    // ... (Lógica cURL para /api/auth/token/ - IDÊNTICA à do front/config.php anterior) ...
                    $token_endpoint = $api_url . '/api/auth/token/';
                    // ... (curl_init, curl_setopt, curl_exec, curl_close) ...
                    $ch = curl_init(); /* ... setopt ... */ $api_response_body = curl_exec($ch); $http_code = curl_getinfo($ch, CURLINFO_HTTP_CODE); $curl_error = curl_error($ch); curl_close($ch);
                    if ($curl_error || $http_code != 200) { /* ... throw exception ... */ }
                    $tokens = json_decode($api_response_body, true);
                    if (empty($tokens['access']) || empty($tokens['refresh'])) { /* ... throw exception ... */ }

                    // ... (Calcular expiração e salvar/atualizar na tabela _auth - IDÊNTICO) ...
                    $expires_timestamp = time() + 3600; $expires_datetime = date('Y-m-d H:i:s', $expires_timestamp);
                    $data_to_save = [ /* ... user, api_url, tokens, expires ... */ ];
                    ...
                    // ... (Lógica para chamar makeAuthenticatedApiRequest('GET', '/api/layouts/') - IDÊNTICA) ...
                    $layouts_from_api = DirectLabelPrinterActions::makeAuthenticatedApiRequest('GET', '/api/layouts/');
                    if (!is_array($layouts_from_api)) { /* ... throw exception ... */ }

                    // ... (Limpar tabela _layouts e inserir novos - IDÊNTICO) ...
                     $DB->query("TRUNCATE TABLE `$layouts_table`");
                     foreach ($layouts_from_api as $layout_data) { /* ... mapear e $DB->insert ... */ }
                    ...
                    // ... (Lógica para pegar 'default_layout_id_api', resetar DB, setar novo, chamar API /selecionar-padrao/ - IDÊNTICA) ...
        ...
        foreach ($layouts_from_db as $layout) { /* ... preencher options e default ... */ }
